...
def purchase(traits, useragent, sessionid, utcnow, anony_id, user_id, cart_id, ordertotal):
    payload = {
        "channel": "web",
        "context": {
            "app": {
    ...
    with open('fake_users.csv', mode='r') as csv_file:
        ...
        for l in csv_file:
            if not skip_header:
                ...
                continue
            if len(l.split(',')) >= 8:
                (user_id, username, email, fname, lname, mobile, gender, age) = l.split(',')
...
if __name__ == '__main__':
    # url = "https://rs.uba.magicwind.top/v1/track"
        ...
        if get_random_num() > 70:
            ...
            if fake.random_int(min=1, max=100) > 50:
                ...
                if fake.random_int(min=1, max=100) > 20:
                    time.sleep(2)
                    purchase(traits, user_agent, session_id, get_utc_now(), anony_id, user_id, cart_id, view_product['price'])
                    print('-> purchase', end='')
