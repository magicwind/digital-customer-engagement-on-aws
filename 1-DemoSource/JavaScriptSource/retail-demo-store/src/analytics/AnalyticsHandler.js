...
export const AnalyticsHandler = {
    ...
    async identify(user) {
        ...
        if (this.rudderstackEnabled()) {
            ...
            window.rudderanalytics.identify(user.id, userProperties);
        }
    ...
    productAddedToCart(user, cart, product, quantity, feature, experimentCorrelationId) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('Product Added', {
                ...
                cart_id: cart.id,
                feature: feature,
                experiment_correlation_id: experimentCorrelationId,
            });
        }
   ...
   async recordShoppingCart (user, cart) {
    ...
    productRemovedFromCart(user, cart, cartItem, origQuantity) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('RemoveFromCart', eventProperties);
        }
    ...
    productQuantityUpdatedInCart(user, cart, cartItem, change) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('UpdateQuantity', eventProperties);
        }
    ...
    productViewed(user, product, feature, experimentCorrelationId, discount) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('View', eventProperties);
        }
    ...
    cartViewed(user, cart, cartQuantity, cartTotal) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('ViewCart', eventProperties);
        }
    ...
    checkoutStarted(user, cart, cartQuantity, cartTotal) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('StartCheckout', eventProperties);
        }
    ...
    orderCompleted(user, cart, order) {
        ...
        if (this.rudderstackEnabled()) {
            // window.rudderanalytics.track('Purchase', eventProperties);
            window.rudderanalytics.track('Checkout Step Completed', {
                checkout_id: cart.id + '-' + order.id,
                step: 1,
            ...
            for (var idx in order.items) {
                rsProducts.push({
                    "product_id": order.items[idx].product_id,
                    "sku": order.items[idx].product_id.toString(),
                    "name": order.items[idx].product_name,
                    "price": +order.items[idx].price.toFixed(2),
                    "quantity": order.items[idx].quantity,
                    "category": order.items[idx].category
                });
            }
    ...
    productSearched(user, query, numResults) {
        ...
        if (this.rudderstackEnabled()) {
            window.rudderanalytics.track('Search', eventProperties);
        }
    ...
    rudderstackEnabled() {
        return process.env.VUE_APP_RUDDERSTACK_WRITE_KEY && process.env.VUE_APP_RUDDERSTACK_WRITE_KEY != 'NONE';
    },
